...
# have a record and are recorded as "0," so make a data frame that has all weeks
# for each combination with a deviation
deviation_combos <- deviation_combos %>% 
  expandRows(deviation_combos, count = num_weeks, count.is.col = FALSE) %>%
  mutate(datetime = seq.Date(from = as.Date(start_date),
